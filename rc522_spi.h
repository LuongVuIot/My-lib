...
#define RC522_ComIEnReg			0x02		// enable and disable interrupt request control bits
#define RC522_DivIEnReg			0x03		// __
...
#define RC522_TReloadHReg			0x2C		// timer reload value higher register
#define RC522_TReloadLReg			0x2D		// timer reload lower register
...
#define RC522_TRANSCEIV				0x0C		// continuously repeat the transmission data from FIFO and the reception data from the RF field.
...
uint8_t RC522_ReadReg(uint8_t addr);															// get register value.
uint8_t RC522_ReadBit(uint8_t addr, uint8_t bit);									// get value of a bit.
...
uint8_t RC522_GetATQA(void);																		// ref AN10834 5/19 Figure1.
void RC522_CS_Write(GPIO_PinState PinState);
RC522Status_e RC522_Request(uint8_t reqMode, uint8_t TagType);
RC522Status_e RC522_Transceive(uint8_t dataSend, uint8_t TxAlign, uint8_t *receiveBuff, uint8_t RxAlign);		// TxAlign: Numbers of last bits will be send, value 0-7; RxAlign: possition of first bit will be stored at, value:0,1,7.
